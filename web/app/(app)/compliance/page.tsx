...
import { getComplianceRecords } from '@/lib/data/compliance';
import type { ComplianceRecord, ComplianceRecordStatus } from '@/types';
...
export default function CompliancePage() {
  ...
  const filteredRecords = useMemo(() => {
    return records.filter((rec) => {
      ...
      const matchesMine =
        mineFilter === 'all' || rec.mineId === mineFilter;
      ...
      if (dueDateFilter !== 'all') {
        ...
        } else if (dueDateFilter === '7days') {
          matchesDueDate = diffDays >= 0 && diffDays <= 7;
        } else if (dueDateFilter === '30days') {
  ...
  const scoredRecords = records.filter(
    (record) => record.complianceScore !== null
  );
    ...
    scoredRecords.length > 0
      ? Math.round(
          scoredRecords.reduce(
            (sum, record) => sum + (record.complianceScore ?? 0),
            0
  ...
  const mineOptions = useMemo(() => {
    ...
    records.forEach((record) => {
      if (!map.has(record.mineId)) {
        map.set(record.mineId, {
          id: record.mineId,
          code: record.mineCode,
          name: record.mineName,
        });
      }
    });
              ...
              <TableBody>
                {filteredRecords.length === 0 ? (
                  <TableRow>
                    ...
                    >
                      No compliance records found matching
                      the current filters.
                    </TableCell>
                  </TableRow>
                ) : (
                    ...
                    >
                      ...
                      <TableCell>
                        <ComplianceRecordStatusBadge
                          status={
                            rec.status as ComplianceRecordStatus
                          }
                        />
                      </TableCell>
                          ...
                          <div className="flex items-center gap-2">
                            <span
                              className={`text-sm font-semibold ${
                                rec.complianceScore === null
                                  ? 'text-muted-foreground'
                                  : scoreColor(rec.complianceScore)
                              }`}
                            >
                              {rec.complianceScore === null
                                ? '—'
